...
Linked lists store sequential (ordered) data in a series of "nodes".  Each node in a linked list contains some data and a reference or "pointer" to the next node.  If the linked list is "doubly linked", each node will also have another pointer to the previous node.  Linked lists that only have pointers to the next node are called "singly linked."
...
1. given a starter data, create a new list,
1. `append` a data node to the end of the list,
1. `prepend` a data node to the start of the list,
